...
const authApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        signup: builder.mutation<{ access_token: string, users: {}, errors: any }, { name: string, email: string | any, password: string, phone: string | any, password_confirmation: string }>({
            query: (credentials) => ({
                url: "/register",
                method: "POST",
                body: credentials
